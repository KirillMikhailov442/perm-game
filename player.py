...
from constants import PLAYER_WIDTH, PLAYER_HEIGHT, PLAYER_DAMAGE, PLAYER_SPEED, PLAYER_HP, PLAYER_DAMAGE_IMAGE_PATH, PLAYER_SOUND_TAKING_DAMAGE_PATH, ARROW_IMAGE_PATH, ARROW_SOUND_PATH, BOMB_COUNT_MAX
...
class Player(pygame.sprite.Sprite):
    ...
    def fire(self, group: pygame.sprite.Group):
        """adding a new arrow to the 'Arrows' group"""
    ...
    def pickup_loot(self, type_loot: str) -> None:
        """pick up loots"""
        if type_loot == 'DAMAGE': self.damage += 0.5
        elif type_loot == "SPEED": self.speed += 0.5
        
        elif type_loot == 'BOMB':
            if self.bomb < BOMB_COUNT_MAX:
                self.bomb += 1

        elif type_loot == 'MEDICINE':
