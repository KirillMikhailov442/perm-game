...
def start_game():
    ...
    while True:
        ...
        # contact of arrows with the tuzemecs
        for arrow in arrows:
        ...
        # contact of arrows with the boss
        if mode_boss:
            for arrow in arrows:
        ...
        # сcontact of tuzemecs with the player
        for tuzemec in tuzemecs:
        ...
        for loot in loots:
            if player.rect.colliderect(loot.rect):
                ...
                if type_loot == 'MEGA_BOMB': tuzemecs.empty()
                else: player.pickup_loot(type_loot)
        ...
        # bomb explosion with tuzemecs
        for bomb in bombs:
            for tuzemec in tuzemecs:
                if bomb.activated:
                    is_in_zone_x, is_in_zone_y = False, False

                    if ((tuzemec.rect.x + tuzemec.width < bomb.rect.x + bomb.width) and (tuzemec.rect.x > bomb.rect.x)): is_in_zone_x = True
                    if ((tuzemec.rect.y + tuzemec.height < bomb.rect.x + bomb.height) and (tuzemec.rect.y > bomb.rect.y)): is_in_zone_y = True                    
                    if (is_in_zone_x and is_in_zone_y): tuzemec.taking_damage(tuzemec.hp, loots, True)

            if bomb.activated: bomb.kill()
