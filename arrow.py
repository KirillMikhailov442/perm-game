...
from constants import ARROW_WIDTH, ARROW_HEGHT, ARROW_SPEED
...
class Arrow(pygame.sprite.Sprite):
    ...
    def moving(self, mouse_pos_x: int, mouse_pos_y: int, area: Tuple[int]) -> None:
        ...
        if self.rect.x <= 0: is_kill = True
        elif self.rect.y <= 0: is_kill = True
        elif self.rect.x + self.width >= area[0]: is_kill = True
        elif self.rect.y + self.height >= area[1]: is_kill = True

        if is_kill: self.kill()
